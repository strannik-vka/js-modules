...
$(document)
    .on('change', '[type="file"]', function() {
        var names = [],
            name_elem = $('[file-change-name="' + $(this).attr('name') + '"]');
        ...
        if (names.length) {
            $(this).parents('.input-group, [file-group]').addClass('active');
        } else {
            $(this).parents('.input-group, [file-group]').removeClass('active');
        }
    })
    .on('click', '[file-change-name] [delete]', function(e) {
        ...
        input.parents('.input-group, [file-group]').removeClass('active');
        input.replaceWith(input.clone().val(''));

        $('[name="' + name_elem.attr('file-change-name') + '"]').trigger('change');
